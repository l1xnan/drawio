...
let enableStoreBkp =
  store != null
    ? store.get("enableStoreBkp") != null
      ? store.get("enableStoreBkp")
      : true
    : false;
...
const codeUrl = url
  .pathToFileURL(codeDir)
  .href.replace(/\/.\:\//, (str) => str.toUpperCase()); // Fix for windows drive letter
// Production app uses asar archive, so we need to go up two more level. It's extra cautious since asar is read-only anyway.
const appBaseDir = path.join(
  __dirname,
  __dirname.endsWith(path.join("resources", "app.asar", "src", "main"))
    ? "/../../../../"
    : "/../../"
);
let appZoom = 1;
// Disabled by default
let isGoogleFontsEnabled =
  store != null
    ? store.get("isGoogleFontsEnabled") != null
      ? store.get("isGoogleFontsEnabled")
      : false
    : false;
...
try {
  if (fs.existsSync(process.cwd() + "/urlParams.json")) {
    let urlParams = JSON.parse(
      fs.readFileSync(process.cwd() + "/urlParams.json")
    );
...
function createWindow(opt = {}) {
  let lastWinSizeStr =
    (store && store.get("lastWinSize")) || "1200,800,0,0,false,false";
  ...
  ipcMain.on("isModified-result", async (e, data) => {
    if (
      ...
      uniqueIsModifiedId != data.uniqueId ||
      modifiedModalOpen
    )
      return null;
    ...
    if (data.isModified) {
      ...
      let response = dialog.showMessageBoxSync(mainWindow, {
        ...
        title: "Confirm",
        message:
          "The document has unsaved changes. Do you really want to quit without saving?", //mxResources.get('allChangesLost')
      });
...
app.whenReady().then(() => {
    ...
    callback({
      responseHeaders: {
        ...
        "Content-Security-Policy": [
          "default-src 'self'; script-src 'self' 'sha256-f6cHSTUnCvbQqwa6rKcbWIpgN9dLl0ROfpEKTQUQPr8=' 'sha256-6g514VrT/cZFZltSaKxIVNFF46+MFaTSDTPB8WfYK+c=' 'sha256-ZQ86kVKhLmcnklYAnUksoyZaLkv7vvOG9cc/hBJAEuQ='; connect-src 'self'" +
  ...
  const pluginsCodeUrl = url
    .pathToFileURL(path.join(getAppDataFolder(), "/plugins/"))
  ...
  session.defaultSession.webRequest.onBeforeRequest(
    ...
    (details, callback) => {
      ...
      if (
        !url.startsWith(codeUrl) &&
        (!isPluginsEnabled() ||
          (isPluginsEnabled() && !url.startsWith(pluginsCodeUrl)))
      ) {
        console.log(
          "Blocked loading file from " + details.url,
          url,
          codeUrl,
          pluginsCodeUrl
        );
  ...
  try {
    ...
    program
      ...
      .argument(
        "[input file/folder]",
        "input drawio file or a folder with drawio files"
      )
      ...
      .option(
        "-x, --export",
        "export the input file/folder based on the given options"
      )
      .option(
        "-r, --recursive",
      ...
      .option(
        ...
        "specify the output file/folder. If omitted, the input file name is used for output with the specified format as extension"
      )
      .option(
        "-f, --format <format>",
      ...
      .option("-t, --transparent", "set transparent background for PNG")
      .option(
        "-e, --embed-diagram",
      ...
      .option("-s, --scale <scale>", "scales the diagram size", parseFloat)
      .option(
        "--width <width>",
      ...
      .option("--crop", "crops PDF to diagram size")
      .option("-a, --all-pages", "export all pages (for PDF format only)")
      .option(
        "-p, --page-index <pageIndex>",
        "selects a specific page (1-based); if not specified and the format is an image, the first page is selected",
        (i) => parseInt(i) - 1
      )
      .option(
      ...
      .option(
        ...
        "selects a page range (1-based, for PDF format only)",
        argsRange
      )
      .option(
        "-u, --uncompressed",
      ...
      .option(
        "-z, --zoom <zoom>",
        "scales the application interface",
        parseFloat
      )
      .option(
        "--svg-theme <theme>",
        "Theme of the exported SVG image (dark, light, auto [default])",
        themeRegExp,
        "auto"
      )
      .option(
        "--svg-links-target <target>",
        "Target of links in the exported SVG image (auto [default], new-win, same-win)",
        linkTargetRegExp,
        "auto"
      )
  ...
  if (options.export) {
    ...
    try {
      ...
      if (options.output) {
        try {
          ...
          if (outStat.isDirectory()) {
            outType = { isDir: true };
          } //If we can get file stat, then it exists
          else {
            throw "Error: Output file already exists";
          }
        } catch (
          e //on error, file doesn't exist and it is not a dir
        ) {
          outType = { isFile: true };
      ...
      if (options.pageIndex != null && options.pageIndex >= 0) {
        ...
        if (rangeFrom >= 0 && rangeTo >= 0 && rangeFrom <= rangeTo) {
          ...
          console.error(
            "Invalid page range: must be non-negative and from ≤ to"
          );
          process.exit(1);
        }
      }
      ...
      if (Array.isArray(paths)) {
        paths = paths.filter(function (path) {
          return path != null && path != "--no-sandbox";
        });
      }
                    ...
                    try {
                      if (data == null || data.length == 0) {
                        ...
                        if (outType != null) {
                          if (outType.isDir) {
                            outFileName =
                              path.join(
                                options.output,
                                path.basename(curFile, path.extname(curFile))
                              ) +
                              "." +
                              format;
                          } else {
                          ...
                          outFileName =
                            path.join(
                              ...
                              path.basename(paths[0], path.extname(paths[0]))
                            ) +
                            "." +
                            format;
                        } //dir
                        ...
                        try {
                          ...
                          if (
                            program.rawArgs.indexOf("-k") > -1 ||
                            program.rawArgs.indexOf("--check") > -1
                          ) {
                          ...
                          fs.writeFileSync(realFileName, data, null, {
                            flag: "wx",
                          });
    ...
    program.rawArgs.indexOf("-V") > -1 ||
    program.rawArgs.indexOf("--version") > -1
  ) {
    //To prevent execution when help/version arg is used
    app.quit();
    return;
  }
  ...
  function checkForUpdatesFn(e) {
    if (e != null && e.senderFrame != null && !validateSender(e.senderFrame))
      return null;
        ...
        submenu: [
          {
            ...
            click() {
              shell.openExternal("https://www.drawio.com");
            },
          },
          {
            label: "Support",
...
function exportDiagram(event, args, directFinalize) {
  ...
  try {
    browser = new BrowserWindow({
      ...
      transparent:
        args.format == "png" && (args.bg == null || args.bg == "none"),
    ...
    contents.on("did-finish-load", function () {
      ...
      function renderingFinishHandler(e, renderInfo) {
        ...
        var pageCount = renderInfo.pageCount,
          bounds = null;
        ...
        if (bounds == null || bounds.width < 5 || bounds.height < 5) {
          //very small page size never return from printToPDF
            ...
            function () {
              browser.capturePage().then(function (img) {
                ...
                var data =
                  args.format == "png"
                    ? img.toPNG()
                    : img.toJPEG(args.jpegQuality || 90);
            ...
            contents
              ...
              .then(async (data) => {
                ...
                if (from < to) {
                  ...
                  data = await mergePdfs(
                    pdfs,
                    args.embedXml == "1" ? args.xml : null
                  );
                  event.reply("export-success", data);
                }
              })
...
function checkFileContent(body, enc) {
  if (body != null) {
    ...
    // image/webp
    if (cc1 == 0x52 && cc2 == 0x49 && cc3 == 0x46 && cc4 == 0x46) {
      //RIFF
      if (cc9 == 0x57 && cc10 == 0x45 && cc11 == 0x42 && cc12 == 0x50) {
        //WEBP
        return true;
      }
    }
...
async function saveDraft(fileObject, data) {
  ...
  if (
    !checkFileContent(data) ||
    path.resolve(draftFileName).startsWith(appBaseDir)
  ) {
...
async function saveFile(fileObject, data, origStat, overwrite, defEnc) {
  if (
    !checkFileContent(data) ||
    path.resolve(fileObject.path).startsWith(appBaseDir)
  ) {
    throw new Error("Invalid file data");
  }
  ...
  var writeFile = async function () {
    ...
    try {
      ...
      fh = await fsProm.open(
        fileObject.path,
        O_SYNC | O_CREAT | O_WRONLY | O_TRUNC
      );
  ...
  if (overwrite) {
    ...
    let stat = fs.existsSync(fileObject.path)
      ? await fsProm.stat(fileObject.path)
      : null;
...
async function writeFile(filePath, data, enc) {
  if (
    !checkFileContent(data, enc) ||
    path.resolve(filePath).startsWith(appBaseDir)
  ) {
    throw new Error("Invalid file data");
  } else {
    let fh;
...
function getAppDataFolder() {
  try {
    ...
    if (!fs.existsSync(drawioDir)) {
      //Usually this dir already exists
...
async function readFile(filename, encoding) {
  ...
  if (
    checkFileContent(data, encoding) &&
