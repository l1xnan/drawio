...
(async function () {
  ...
  Menus.prototype.init = function () {
    ...
    editorUi.actions.put(
      "useOffline",
      new Action("useOffline" + "...", function () {
    ...
    this.put(
      ...
      new Menu(
        mxUtils.bind(this, function (menu, parent) {
          ...
          this.addMenuItems(
            menu,
            ["-", "synchronize", "-", "save", "saveAs", "-", "import"],
            parent
          );
  ...
  EditorUi.prototype.init = async function () {
    ...
    electron.registerMsgListener("isModified", (uniqueId) => {
      ...
      if (currentFile != null) {
        ...
        reply.draftPath =
          EditorUi.enableDrafts && currentFile.fileObject
            ? currentFile.fileObject.draftFileName
            : null;
      }
        ...
        if (editorUi.getCurrentFile() != null) {
          ...
          if (paths !== undefined && paths[0] != null) {
            ...
            localStorage.setItem(
              ".lastImpDir",
              await requestSync({ action: "dirname", path: path })
            );
            var asImage =
              /\.png$/i.test(path) ||
              /\.gif$/i.test(path) ||
            ...
            if (
              ...
              electron.request(
                { action: "readFile", filename: path, encoding: encoding },
                mxUtils.bind(this, function (data) {
                  try {
                    if (editorUi.isLucidChartData(data)) {
                      editorUi.convertLucidChart(
                        data,
                        function (xml) {
                      ...
                      editorUi.importVisio(
                        editorUi.base64ToBlob(data, "application/octet-stream"),
                        function (xml) {
                          editorUi.spinner.stop();
                          graph.setSelectionCells(editorUi.importXml(xml));
                        }
                            ...
                            function (data2, w, h) {
                              ...
                              graph.setSelectionCell(
                                graph.insertVertex(
                                  ...
                                  w,
                                  h,
                                  "shape=image;aspect=fixed;image=" +
                                    editorUi.convertDataUri(data2) +
                                    ";"
                                )
                              );
                            },
        ...
        new PluginsDialog(
          ...
          async function (callback) {
            ...
            if (await requestSync("isPluginsEnabled")) {
              var extPluginsBtn = mxUtils.button(
                mxResources.get("selectFile") + "...",
                async function () {
                  var warningMsgs = mxResources
                    .get("pluginWarning")
                    .split("\\n");
                  ...
                  var paths = await requestSync({
                    action: "showOpenDialog",
                    defaultPath:
                      lastDir || (await requestSync("getDocumentsFolder")),
                    filters: [
  ...
  App.prototype.loadArgs = function (argsObj) {
    ...
    if (
      ...
      var success = mxUtils.bind(
        this,
        function (fileEntry, data, stat, name, isModified) {
          this.spinner.stop();
        ...
        await requestSync({
          ...
          listener: mxUtils.bind(this, function (curr, prev) {
            ...
            if (
              ...
              if (
                file.unwatchedSaves ||
                (file.stat != null && file.stat.mtimeMs == curr.mtimeMs)
              ) {
              ...
              file.addConflictStatus(
                null,
                mxUtils.bind(this, function () {
                  file.ui.updateStatus(
                    mxUtils.bind(this, function () {
                      file.ui.editor.setStatus(
  ...
  App.prototype.pickFile = function () {
    var doPickFile = mxUtils.bind(this, function () {
      this.chooseFileEntry(
        mxUtils.bind(this, function (fileEntry, data, stat, name, isModified) {
  ...
  App.prototype.chooseFileEntry = async function (fn) {
      ...
      filters: [
        {
          ...
          extensions: ["drawio", "xml", "png", "svg", "html"],
        },
    ...
    if (paths !== undefined && paths[0] != null) {
      localStorage.setItem(
        ".lastOpenDir",
        await requestSync({ action: "dirname", path: paths[0] })
      );
  ...
  EditorUi.prototype.normalizeFilename = function (title, defaultExtension) {
    ...
    if (
      tokens.length == 1 ||
      mxUtils.indexOf(["xml", "html", "drawio", "png", "svg"], ext) < 0
    ) {
  ...
  App.prototype.readGraphFile = function (fn, fnErr, path, noDraftCheck) {
    ...
    var readData = mxUtils.bind(this, function (data) {
      ...
      if (isVsdx) {
        ...
        var tmp = Editor.extractGraphModelFromPdf(
      ...
      electron.request(
        { action: "fileStat", file: path },
        mxUtils.bind(this, function (stat_p) {
          stat = stat_p;
          fn(fileEntry, data, stat, null, false);

          electron.request(
            { action: "isFileWritable", file: path },
            mxUtils.bind(this, function (isWritable) {
              if (!isWritable) {
                var file = this.getCurrentFile();
  ...
  LocalFile.prototype.isConflict = function (stat) {
    return (
      stat != null && this.stat != null && stat.mtimeMs != this.stat.mtimeMs
    );
  };

  LocalFile.prototype.saveFile = async function (
    revision,
    success,
    error,
    unloading,
    ...
    if (!this.savingFile) {
      ...
      try {
        if (this.fileObject == null) {
          ...
          var name = this.ui.normalizeFilename(
            this.getTitle(),
  ...
  LocalFile.prototype.saveAs = async function (title, success, error) {
    try {
      var lastDir =
        this.fileObject != null && this.fileObject.path != null
      ...
      var path = await requestSync({
        action: "showSaveDialog",
        defaultPath:
          (lastDir || (await requestSync("getDocumentsFolder"))) + "/" + name,
        filters: this.ui.createFileSystemFilters(ext),
      });
  ...
  LocalFile.prototype.removeDraft = async function () {
    try {
      if (this.fileObject == null) {
        ...
        await requestSync({
          action: "deleteFile",
          file: this.fileObject.draftFileName,
        });
      }
    } catch (e) {
      // ignore
   ...
   */
  EditorUi.prototype.writeImageToClipboard = async function (
    dataUrl,
    w,
    h,
    error
  ...
  EditorUi.prototype.saveData = async function (
    filename,
    format,
    data,
    mimeType,
      ...
      mxUtils.bind(this, async function () {
        ...
        switch (format) {
          ...
          case "pdf":
            ...
            break;
          case "vsdx":
            filters = [{ name: "VSDX Documents", extensions: ["vsdx"] }];
            break;
          case "html":
